...
char* c_fillment(struct formating* new_format, int arg) {
    ...
    char *formatted_c = (char*)x_calloc(width + 2, sizeof(char));  // буфер длины ширина + arg
    formatted_c[0] = arg;
...
char* c_fillment_wchar(struct formating* new_format, char* arg, int *count_bytes) {
    ...
    int i = 0;
    char *formatted_c = (char*)x_calloc(width + len + 1, sizeof(char));  // буфер длины ширина + arg
    for (; i < len; i++) {
        formatted_c[i] = arg[i];
        (*count_bytes)++;
    }
